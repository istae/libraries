...
char_freq* char_frequency(int* str, int str_len, char_freq* cf, int* cf_len)
{
    for (int i=0; i < str_len; i++) {
        if (str[i] < 0) str[i] += 512;
        cf[str[i]].value = str[i];
        cf[str[i]].freq++;
    }

    // sort the frequency of bytes from low to high
    qsort(cf, 512, sizeof(char_freq), __char_frequency_sort_comp);

    // return the first non zero frequency byte index
    int i;
    for(i=0; i < 512 && cf[i].freq == 0; i++)
        ;
    *cf_len = 512 - i;
    return i + cf;
}
...
huffman build_huffman_tree(char_freq* cf, int cf_len)
{
    ...
    huff_code* codes = malloc(cf_len*sizeof(huff_code));
